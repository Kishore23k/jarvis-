...
# Tab names matching your existing Jarvis_Memory sheet tabs
SECTOR_TAB_MAP = {
    ("realestate",    "lead"):     "RealEstate",
    ("realestate",    "reminder"): "RealEstate",
    ("manufacturing", "stock"):    "Manufacturing",
    ("manufacturing", "reminder"): "Manufacturing",
    ("accounts",      "invoice"):  "Accounts",
    ("accounts",      "reminder"): "Accounts",
    ("software",      "ticket"):   "Software",
    ("software",      "reminder"): "Software",
}
SECTOR_PREFIX = {
    "realestate":    "RE",
    "manufacturing": "MFG",
...
def _tab_name(sector: str, record_type: str) -> str:
    """Returns existing sheet tab name for this sector+record_type."""
    return SECTOR_TAB_MAP.get((sector, record_type),
           f"{SECTOR_PREFIX.get(sector, sector.upper()[:3])} - {RECORD_MAP.get(record_type, (record_type.title(),[]))[0]}")
